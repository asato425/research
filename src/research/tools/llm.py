...
class LLMTool:
    ...
    def create_model(self, model_name: str = "gemini", temperature: float = 0.0, output_model: any = None) -> any:
        ...
        """
        models = {
            "gemini": lambda: ChatGoogleGenerativeAI(model="gemini-1.5-flash", temperature=temperature).with_structured_output(output_model) if output_model else ChatGoogleGenerativeAI(model="gemini-1.5-flash", temperature=temperature),
            ...
            "gpt-5": lambda: ChatOpenAI(model="gpt-5-nano", temperature=temperature).with_structured_output(output_model) if output_model else ChatOpenAI(model="gpt-5-nano", temperature=temperature),
