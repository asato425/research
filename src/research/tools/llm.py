...
class LLMTool:
    ...
    def create_model(self, model_name: str = "gemini", temperature: float = 0.0, output_model: any = None) -> any:
        ...
        models = {
            ...
            "gpt-4o-mini": lambda: ChatOpenAI(model="gpt-4o-mini", temperature=temperature).with_structured_output(output_model) if output_model else ChatOpenAI(model="gpt-4o-mini", temperature=temperature),
            "gpt-5-mini": lambda: ChatOpenAI(model="gpt-5-mini", temperature=temperature).with_structured_output(output_model) if output_model else ChatOpenAI(model="gpt-5-mini", temperature=temperature),
            "gpt-5": lambda: ChatOpenAI(model="gpt-5", temperature=temperature).with_structured_output(output_model) if output_model else ChatOpenAI(model="gpt-5", temperature=temperature),
