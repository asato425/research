...
class GitHubAPIClient:
    ...
    def __init__(self, repo_url: str, base_url: str = "http://localhost:8000", local_path: str = None):
        ...
        _start_github_api_server()


    def delete_cloned_repository(self) -> dict:
        """
        ...
        if not os.path.exists(self.local_path):
            return {"status": "not_found", "message": f"{self.local_path} は存在しません。", "path": self.local_path}
        try:
            shutil.rmtree(self.local_path)
            return {"status": "success", "message": f"Deleted: {self.local_path}", "path": self.local_path}
        except Exception as e:
            return {"status": "error", "message": str(e), "path": self.local_path}
    def create_empty_file(self, filename: str) -> dict:
        """
        ...
        if os.path.exists(file_path):
            return {"status": "exists", "message": f"{file_path} は既に存在します。", "path": file_path}
        try:
            with open(file_path, "w"):
                pass
            return {"status": "success", "message": f"Created empty file: {file_path}", "path": file_path}
        except Exception as e:
    ...
    def clone_repository(self, local_dir: str = None):
        """
        ...
        if os.path.exists(local_dir):
            ...
            return {"status": "exists", "message": f"{local_dir} は既に存在します。", "path": local_dir}
        try:
            subprocess.run(["git", "clone", self.repo_url, local_dir], check=True)
            self.local_path = local_dir
            return {"status": "success", "message": f"Cloned to {local_dir}", "path": local_dir}
        except subprocess.CalledProcessError as e:
            return {"status": "error", "message": str(e), "path": local_dir}
